...
static int		file_parce(char **map, t_win *win, int size)
{
	...
	while (map[i] && map[i][0] != ' ' && !err && map[i][0] != '1')
	{
		...
		if (!(err = valid_identifier(map[i])))
			identifier(map[i++], win);
		if (err)
			invalid_file(3);
	}
	if (!err && check_map(map, i, size, win))
		invalid_file(4);
	return (err || check_map(map, i, size, win));
}
...
static int		lst_to_str(t_list *file_lst, int size, t_win *win)
{
	...
	while (!err && file_lst)
	{
		if (!(map[i] = ft_calloc(ft_strlen(file_lst->content), sizeof(char))))
			err = 1;
		map[i++] = file_lst->content;
		file_lst = file_lst->next;
	}
	err = (file_parce(map, win, size)) ? 1 : err;
	return (err);
}
